...
public class ClientTransactionRepository : IClientTransactionRepository
{
    ...
    public Task<List<ClientTransaction>> GetByPeriod(DateTime dateFrom, DateTime dateTo, CancellationToken cancellationToken = default)
    {
        return _context.ClientTransactions.Where(x => x.DateOccured > dateFrom && x.DateOccured < dateTo)
            .ToListAsync(cancellationToken);
    }

    public Task<ClientTransaction?> GetLatest(CancellationToken cancellationToken = default)
    {
        return _context.ClientTransactions.OrderBy(x => x.DateOccured).FirstOrDefaultAsync(cancellationToken);
    }
